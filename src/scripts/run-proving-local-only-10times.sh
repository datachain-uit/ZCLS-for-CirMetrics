#!/bin/bash
# filepath: /mnt/d/vscode/ZCLS-for-CirMetrics/src/scripts/run-proving-local-only-10times.sh

if [ $# -lt 4 ]; then
  echo "Usage: $0 CIRCUIT_NAME BATCH_SIZE SIMP_LEVEL METHOD"
  echo "Example: ./run-proving-local-only-10times.sh RollupValidator 8 o1 groth16"
  exit 1
fi
...
LOGFILE="logs/run_proving_local_only_${CIRCUIT_NAME}_batch${BATCH_SIZE}_${SIMP_LEVEL}_${METHOD}_10runs.log"

echo "Running proving-local-only.sh 10 times" | tee $LOGFILE
