...
import "./RollupValidator.sol";
import "./ExitValidator.sol";
...
contract Rollup is Ownable, ReentrancyGuard {
    ...
    RollupValidator immutable rollupValidator;
    ExitValidator immutable exitValidator;
    ...
    constructor(IERC20 _token, IHasher2 _hasher2, IHasher3 _hasher3) Ownable(msg.sender) {
        ...
        rollupValidator = new RollupValidator();
    ...
    function update(
        ...
        require(rollupValidator.verifyProof(a, b, c, [uint(_txRoot), uint(_oldRoot), uint(_newRoot)]), "Update does not drive oldRoot to newRoot");
    ...
    function withdraw(
        ...
        require(exitValidator.verifyProof(a, b, c, [uint(_tx), uint(_txRoot), uint(_amount)]), "Withdraw transaction is not present on txRoot");
